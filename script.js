 ...
 * quantity of that product. In both cases the function ends up calling the "updatePrice(productName)" and "calculateTotal()" functions.
 * @param {String} productName - This is the product name added to the "basketProducts" array.
 * @param {Number} productPrice - This is the product price added to the "basketPrices" array.
 * @param {Number} productAmount - This is the product quantity added to the "basketAmounts" array.
 */
function addProductToBasket(productName, productPrice, productAmount) {
    let index = basketProducts.indexOf(productName);
    //If the added product ("productName") is not yet in the "basketProducts" array...
    if (index == -1) {
    ...
    //If the added product ("productName") is already in the "basketProducts" array...
    } else {
        //Its quantity is increased by 1 (it is not necessary to add a new product with the same name).
        basketAmounts[index]++;
    }
 ...
 * product by 1. It then calls the "updatePrice(productName)" and "calculateTotal()" functions.
 * @param {String} productName - This is the product whose quantity is reduced by 1.
 */
function removeProductFromBasketOnce(productName) {
    let index = basketProducts.indexOf(productName);
    //If the product that corresponds to the "-" button that the user has clicked on ("productName") is no longer in the "basketProducts" 
    //array...
    if (index == -1) {
        //The function stops, otherwise the quantity of another product would be reduced by 1.
        return;
    //If the product that corresponds to the "-" button that the user has clicked on ("productName") is in the "basketProducts" array...
    } else {
 ...
 * "updatePrice(productName)" and "calculateTotal()" functions.
 * @param {String} productName - This is the product whose name, price and quantity are removed from the 3 arrays respectively.
 */
...
/**
 * This function, called whenever the "addProductToBasket(productName, productPrice, productAmount)", the 
 * "removeProductFromBasketOnce(productName)" or the "removeProductFromBasket(productName)" function is called, calculates the total number 
 * of products in the order and the final price of it. To do this, it iterates through the "basketAmounts" array (although it could iterate 
 * through any of the other two arrays as well, since the length of all three is the same) and adds up all the quantities and prices entered 
 * by the user. It is important to highlight that, when adding up all the prices of the "basketPrices" array, the function multiplies each 
 * element of this array by its corresponding quantity (that is, by the element with the same index of the "basketAmounts" array). Finally, 
 * the function calls the "checkTotalPrice(sumOfPrices)" and the "checkBasketStatus(sumOfPrices)" functions.
 */
...
/**
 * This function, called whenever the "calculateTotal()" function is called, checks if the varibale "sumOfPrices" (belonging to the function 
 * "calculateTotal()") is equal or greater than 50. Depending on this, the function displays the "deliveryCostsRow" and the 
 * "deliveryCostsInfo" elements or not.
 * @param {Number} condition - This is the variable "sumOfPrices".
 */
function checkTotalPrice(condition) {
    //If "sumOfPrices" ("condition") is less than 50...
    if (condition < 50) {
    ...
    //If "sumOfPrices" ("condition") is equal or greater than 50...
    } else {
        //Both the message telling the user that the minimum order to avoid delivery costs must be 50 euros and the "deliveryCostsRow"  
        //element in the total section are no longer displayed in the basket.
 ...
 * To do this, it checks if the varibale "sumOfPrices" (belonging to the function "calculateTotal()") is equal to 0. If so, the 
...
function checkBasketStatus(condition) {
    //If "sumOfPrices" ("condition") equals to 0...
    if (condition == 0) {
        //The message telling the user that his/her basket is empty is displayed.
        basketStatusInfo.classList.remove('d-none');
    //If "sumOfPrices" ("condition") is more than 0...
    } else {
        //The message telling the user that his/her basket is empty is no longer displayed.
...
/**
 * This function, called whenever the "addProductToBasket(productName, productPrice, productAmount)", the 
 * "removeProductFromBasketOnce(productName)" or the "removeProductFromBasket(productName)" function is called, updates the quantity 
 * and price of the "productName" in the basket.
 * @param {String} productName - This is the name of the product whose quantity and price are updated in the basket.
 */
function updatePrice(productName) {
    let index = basketProducts.indexOf(productName);
    let total = document.getElementById('total');
    let productRow = document.getElementById(`${productName}-row`);
    //If the product selected by the user ("productName") is no longer in the "basketProducts" array...
    if (index == -1) {
        //And there is no other element in the "basketProducts" array...
        if (basketProducts.length == 0) {
            //Both the "productRow" element and the total section are no longer displayed in the basket.
            ...
            //product selected by the user ("productName") is no longer displayed.
            productRow.classList.add('d-none');
        }
    //If the product selected by the user ("productName") is already in the "basketProducts" array...
    } else {
        ...
        //Both the quantity of the product and its price are updated in the basket.
 ...
 * @param {String} productName - This is the name of the product that is displayed in the basket.
 */
